...
public class BestOfferService {
	...
	private Double calculateDeliveryTax(Cart cart) {
		...
		Double totalWeight = cart.getItems().stream().reduce(0d, (acc, item) -> acc + item.getWeight(), Double::sum);
	...
	private Double calculateItemsPriceWithCategoryDiscount(Cart cart) {
		return cart.getItems().stream().reduce(0d,
				(acc, item) -> acc + (item.getValue() * (discountsByCategory.get(item.getCategory()) != null
